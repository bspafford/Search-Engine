...
bool IsImportantWord(const std::string_view& word) {
    return !stopWords.count(std::string(word));
}
...
void EndTimer(const std::string& debugStr) {
    auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - durationTime);
...
std::string PrettyPrint(long num) {
