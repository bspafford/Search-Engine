...
void Renderer::FinishedSetup() {
    // something here to tell main that we are good to go
    std::cout << "finished setup\n";
    finishedSetup = true;
}

void Renderer::NavigatePage(ix::WebSocket& webSocket, const std::string& url) {
...
void Renderer::EvaluateHTML(ix::WebSocket& webSocket) {
...
void Renderer::RenderedHTML(const nlohmann::json& json) {
    htmlBody = json["result"]["result"]["value"].get<std::string>();
    gettingHTML = false;
}

void Renderer::WaitFor(bool& con, const std::string& debugStr) {
    int sleepTime = 0;
...
bool Renderer::HasEnoughContent(const std::string& html) {
    lxb_status_t status;
    ...
    bool title = HasTitle();
    bool description = HasDescription();
    bool favicon = HasFavicon();
...
std::string Renderer::GetHTML(const std::string& url, long* httpCode) {
    Helper::StartTimer("Getting HTML");

    std::string html = CurlGet(url, httpCode, "GET");
...
void Renderer::StartClient(const std::string& debuggerUrl) {
    // connect to a server
...
std::string Renderer::CurlGet(const std::string& url, long* httpCode, const std::string& method) {
    std::cout << "\033[31mstarting curl: " << std::this_thread::get_id() << "\033[0m\n";
    std::string html = "";
    CURLcode res;
    ...
    std::cout << "\033[31mending curl: " << std::this_thread::get_id() << "\033[0m\n";
    return html;
}

void Renderer::LaunchChromium() {
    std::this_thread::sleep_for(std::chrono::milliseconds(1000)); // temp

    std::string data = CurlGet("127.0.0.1:9222/json/new", nullptr, "PUT");

    std::cout << "data: " << data << "\n";
    nlohmann::json json = nlohmann::json::parse(data);

    StartClient(json["webSocketDebuggerUrl"]);
